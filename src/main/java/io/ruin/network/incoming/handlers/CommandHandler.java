...
import io.ruin.model.activities.pestcontrol.PestControl;
...
import static io.ruin.model.activities.tournament.Tournament.LEAVE_BOUNDS;
import static io.ruin.model.activities.tournament.Tournament.TOURNAMENT_BOUNDS;
...
public class CommandHandler implements Incoming {
    ...
    private static boolean handleRegular(Player player, String query, String command, String[] args) {
        switch(command) {
             ...
             */
            case "barrage": {
                if(World.isEco() && !player.isPKMode()) {
                    player.dialogue(new MessageDialogue("You need to be playing the PK gamemode to use this command."));
                    return true;
                }
            ...
            case "enterraids":
            case "raidstest":
                player.getMovement().teleport(1254, 3558);
                return true;
            case "setlvls":
            case "setlvl":
            case "setlevels":
            ...
            case "heal": {
                ...
                if (!player.isAdmin() && (!player.isDonator() || player.isGroup(PlayerGroup.DONATOR))) {
            ...
            case "donatorszone":
            case "donorzone":
                player.dialogue(new NPCDialogue(2108, "The donator zone is available through the portal, just west of the Edgeville bank."));
                break;
            case "skull": {
            ...
            case "drag": {
                ...
                player.dialogue(new MessageDialogue("You can change your drag setting inside the journal -> toggles interface."));
                return true;
            }
            case "achievements": {
                if(World.isPVP())
            ...
            case "yell": {
                ...
                if(player.isAdmin() || player.isSupport() || player.isModerator() || player.isSeniorModerator()) {
                    ...
                    bypassFilter = false;
                    delaySeconds = 15;
                } else if(player.isGroup(PlayerGroup.MASTER_DONATOR) || player.isGroup(PlayerGroup.YOUTUBER) || player.isGroup(PlayerGroup.BETA_TESTER) || player.isGroup(PlayerGroup.DICE_HOST)) {
                    bypassFilter = false;
                    delaySeconds = 30;
                } else if(player.isGroup(PlayerGroup.ULTIMATE_DONATOR)) {
                    bypassFilter = false;
                    delaySeconds = 45;
                ...
                PlayerGroup clientGroup = player.getClientGroup();
                   message = Color.BLUE.wrap("[" + (clientGroup.clientImgId != -1 ? clientGroup.tag() + " " : "") + player.getName() + "]") + " " + message;
            ...
            case "updates": {
                player.openUrl(World.type.getWorldName() + " Updates", World.type.getWebsiteUrl() + "/forums/index.php");
                return true;
            }
            case "rules": {
                player.openUrl(World.type.getWorldName() + " Rules", World.type.getWebsiteUrl() + "/forums/index.php");
                return true;
            }
            case "vote": {
            ...
            case "stream": {
                if(World.isPVP())
                    return true;
                player.openUrl(World.type.getWorldName() + " Stream", "https://www.youtube.com/channel/");
                return true;
            }
            case "guides": {
                player.openUrl(World.type.getWorldName() + " Guides", World.type.getWebsiteUrl() + "/forums/");
                return true;
            }
            case "support": {
                player.openUrl(World.type.getWorldName() + " Support", World.type.getWebsiteUrl() + "/forums/");
                return true;
            }
            case "forums": {
                player.openUrl(World.type.getWorldName() + " Forums", World.type.getWebsiteUrl() + "/forums/index.php");
                return true;
            }
            case "hiscores":
            case "scores": {
            ...
            case "discord": {
                player.openUrl("Official " + World.type.getWorldName() + " Discord Server", "https://discord.gg/kneKE8J");
                return true;
            }
            case "thread": {
                int id;
                ...
                player.openUrl(World.type.getWorldName() + " Thread #" + id, World.type.getWebsiteUrl() + "/forums/index.php?app=forums&module=forums&controller=topic&id=" + id);
                return true;
            }
            case "member": {
                int id;
                ...
                player.openUrl(World.type.getWorldName() + " Member #" + id, World.type.getWebsiteUrl() + "/forum/index.php?app=core&module=members&controller=profile&id=" + id);
                return true;
            }
            case "referral": {
    ...
    private static boolean handleAdmin(Player player, String query, String command, String[] args) {
        ...
        switch(command) {
            ...
            case "removedeeppker":
                ...
                return true;
            case "toggledmm":
                Tournament.TOGGLE_SPECIAL = !Tournament.TOGGLE_SPECIAL;
                player.sendMessage("Tournament is now: " + (Tournament.TOGGLE_SPECIAL ? "using special dmm sets" : " not using special dmm sets"));
                return true;
            case "fetchupdate":
                LatestUpdate.fetch();
                return true;
            ...
            case "test": {
                for (int i = 0; i < 2000; i ++)
                    player.getPacketSender().sendItems(-1, -1, i, new Item(4151, i), new Item(20997, i), new Item(6570, i));
                return true;
            }
            case "inferno": {
            ...
            case "settournytime": {
                ...
                Tournament.ticksUntilStart = Server.getEnd(Server.toTicks(startTime));
                return true;
            }
            case "starttourny": {
            ...
            case "endtourny": {
                World.players.forEach(players -> {
                    if (players.getPosition().inBounds(TOURNAMENT_BOUNDS)) {
                        World.startEvent(e -> {
                            players.lock();
                            TournamentViewingOrb.reset(players);
                            players.animate(1816);
                            players.graphics(342);
                            e.delay(2);
                            players.getMovement().teleport(LEAVE_BOUNDS);
                            players.resetAnimation();
                            e.delay(2);
                            players.unlock();
                        });
                    }
                });
                Tournament.tournamentInProgress = false;
                return true;
            }
            case "staffbounty": {
            ...
            case "toregion": {
                ...
                player.getMovement().teleport(x, z, player.getHeight());
                return true;
            }
            case "clipping": {
