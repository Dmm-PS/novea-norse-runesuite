...
public class Trade extends ItemContainer {
    ...
    public void tradeRustin() {
        Trade rustinTrade = new Trade(null);
        rustinTrade.rustin = true;
        init(rustinTrade);
    }
    ...
    private void updateRustin() {
        long priceOfItem = 0L;
        for(Item item : getItems()) {
            if(item != null)
                priceOfItem += Rustin.getResellPrices(item) * item.getAmount();
        }
        Item tickets = targetTrade.get(0);
        if(priceOfItem == 0) {
            if(tickets == null)
                return;
            tickets.remove();
        } else {
            if(priceOfItem > Integer.MAX_VALUE)
                priceOfItem = Integer.MAX_VALUE;
            if(tickets == null)
                targetTrade.set(0, new Item(5020, (int) priceOfItem));
            else
                tickets.setAmount((int) priceOfItem);
        }
        targetTrade.sendUpdates(this);
    }
    ...
    private void open() {
        stage = 1;
        sendAll = true;
        String targetName = targetTrade.sigmund ? "Sigmund The Merchant" : targetTrade.rustin ? "Rustin" : targetTrade.player.getName();
    ...
    private void updateSlots() {
        ...
        else if(targetTrade.rustin)
            player.getPacketSender().sendString(335, 9, "Offer items you would like to sell to Rustin.");
        else
    ...
    public void close() {
        ...
        if(!targetTrade.sigmund && !targetTrade.rustin) {
    ...
    public void offer(Item item, int amount) {
        ...
        if(targetTrade.sigmund) {
            ...
            if (item.getDef().isNote()) {
                sendMessage("Rustin does not accept noted items.");
                return;
            } else if (Rustin.getResellPrices(item) == 0 || !item.getDef().tradeable || item.getDef().free) {
                sendMessage("Rustin has no interest in buying that item.");
                return;
            }
    ...
    private void second() {
        ...
        player.getPacketSender().sendString(334, 30, "Trading with:<br>" + (targetTrade.sigmund ? "Sigmund The Merchant" : targetTrade.rustin ? "Rustin" : targetTrade.player.getName()));
        player.getPacketSender().sendClientScript(917, "ii", -1, -1);
    ...
    private void destroy(boolean returnItems) {
        if(returnItems) {
            ...
            Item tickets = targetTrade.get(0);
            if(tickets != null)
                player.getInventory().add(tickets);
            sendMessage("Your trade with Rustin the Merchant was successful.");
        } else {
            /**
             * Trade my items
             */
