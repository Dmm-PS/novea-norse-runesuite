...
public class Horvik {
    ...
    static {
        /**
         * Refund upgrade scroll
         */
        ItemAction.registerInventory(786, "read", (player, item) -> {
            item.remove();
            player.getInventory().add(13307, 5000);
            player.dialogue(new ItemDialogue().one(13307, "You have been refunded 5,000 bm for your scroll.<br>Item upgrades are now done from Horvik."));
        });
        /**
         * Upgrading
         */
        ...
        for(ItemUpgrading upgrade : ItemUpgrading.values()) {
            ...
            ItemNPCAction.register(upgrade.regularId, 535, (player, item, npc) -> {
        ...
        NPCAction.register(535, "upgrade-items", (player, item) -> player.sendScroll("<col=800000>Item Upgrades", upgradeInfoArray));
        /**
         * Repairing
         */
        NPCAction.register(535, "repair-items", Horvik::repairAll);
        for(ItemBreaking brokenItem : ItemBreaking.values()) {
            ...
            ItemNPCAction.register(brokenItem.brokenId, 535, (player, item, npc) -> repairItem(player, item, brokenItem));
        }
        /**
         * Zamorakian spear/hasta conversion
         */
        for(int id : new int[]{11824, 11889})
            ItemNPCAction.register(id, 535, Horvik::convertZamorakianWeapon);
    }
