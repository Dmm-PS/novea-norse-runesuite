    ...
    public void startDeath(Hit killHit) {
        ...
        npc.startEvent(event -> {
            ...
            if (killer != null && killer.player != null) {
                ...
                //Donator Boss
                if (id == 7411 || id == 7412 || id == 7414 || id == 7415) {
                ...
                int wildernessPoints = 0;

                // Callisto, Venenatis, Vet'ion:
                if (id == 6503 || id == 6609 || id == 6504 || id == 6610 || id == 6611 || id == 6612) {
                    wildernessPoints = 80;
                }
                // Chaos Elemental, Crazy Archaeologist, Chaos Fanatic, Skorpia:
                else if (id == 2054 || id == 6505 || id == 6618 || id == 6619 || id == 6615) {
                    wildernessPoints = 35;
                }

                if (wildernessPoints > 0) {
                    killer.player.wildernessPoints += wildernessPoints;
                    killer.player.sendFilteredMessage("You receive <col=6f0000>" + wildernessPoints + "</col> wilderness points for killing " + npc.getDef().name + ". You now have " + NumberUtils.formatNumber(killer.player.wildernessPoints) + " points.");
                }
            }
    ...
    public void dropItems(Killer killer) { //todo - wanna just change this to Player pKiller ?
        ...
        if(t != null) {
            ...
            for(int i = 0; i < rolls; i++) {
                ...
                if(items != null) {
                    for(Item item : items) {
                        //Change rune drops to rune g
                        switch (item.getId()) {
                            case 1163: //Rune full helm
                                item.setId(2619);
                                break;
                            case 1164: //Rune full helm noted
                                item.setId(2620);
                                break;
                            case 1127: //Rune pl8
                                item.setId(2615);
                                break;
                            case 1128: //Rune pl8 noted
                                item.setId(2616);
                                break;
                            case 1079: //Rune pl8legs
                                item.setId(2617);
                                break;
                            case 1080: //Rune pl8legs noted
                                item.setId(2618);
                                break;
                            case 1093: //Rune pl8skirt
                                item.setId(3476);
                                break;
                            case 1094: //Rune pl8skirt noted
                                item.setId(3477);
                                break;
                            case 1201: //Rune kite
                                item.setId(2621);
                                break;
                            case 1202: //Rune kite noted
                                item.setId(2622);
                                break;
                        }
                        if (npc.getDef().name.contains("gorilla")
                                && npc.wildernessSpawnLevel < 1
                                && (item.getId() == 22613
                                || item.getId() == 22622
                                || item.getId() == 22647
                                || item.getId() == 22636
                                || item.getId() == 22634
                                || item.getId() == 5020
                                || (item.getId() == 995 && (item.getAmount() >= 50000 && item.getAmount() <= 250000)))) {
                            continue;
                        }
        ...
        if (pKiller != null && pKiller.vorkathKills.getKills() >= 50 && !pKiller.obtained50KCVorkathHead) {
            Item item = new Item(2425);
            GroundItem groundItem = new GroundItem(item).position(dropPosition);
            groundItem.spawn();
