...
public class Rustin {
    ...
    static {
        NPCAction.register(315, "sell-items", (((player, npc) -> {
            if (player.getGameMode().isIronMan()) {
                player.sendFilteredMessage("Ironman cannot trade Rustin");
                return;
            }
            player.getTrade().tradeRustin();
        })));
        ItemNPCAction.register(5020, 315, ((player, item, npc) -> convertTicketsToPoints(player, item)));
    ...
    private static void convertTicketsToPoints(Player player, Item item) {
        item.remove();
        int amount = item.getAmount();
        player.wildernessPoints += amount;
        player.sendFilteredMessage("You have converted " + Color.RED.wrap(amount + "") + " tickets into wilderness points");
    }
