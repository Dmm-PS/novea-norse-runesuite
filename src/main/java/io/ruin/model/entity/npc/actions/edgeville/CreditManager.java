...
public class CreditManager {
        ...
        player.dialogue(
                ...
                new OptionsDialogue(
                        ...
                        new Option("What are the different tiers of donators?", () -> player.dialogue(
                                ...
                                new NPCDialogue(npc, "Afterward, you'll find the Ultimate Donators, a growingly prestigious rank, and their names will be next to a " + PlayerGroup.ULTIMATE_DONATOR.tag() + " icon."),
                                new NPCDialogue(npc, "The next rank is quite excellent, and a personal favorite of mine, and that's the Master Donators. You'll find their names beside a " + PlayerGroup.MASTER_DONATOR.tag() + " icon."),
    ...
    private static void openStore(Player player, NPC npc) {
        player.dialogue(
                new OptionsDialogue("Would would you like to open?",
                        new Option("Refunded Credits Shop", () -> npc.getDef().shop.open(player)),
                        new Option("Web Store", () -> player.openUrl(World.type.getWorldName() + " Store", STORE_URL))
                )
        );
    }
    ...
    public static PlayerGroup getGroup(Player player) {
        ...
        if(spentDollars >= 500)
            return PlayerGroup.MASTER_DONATOR;
        if(spentDollars >= 250)
            return PlayerGroup.ULTIMATE_DONATOR;
        if(spentDollars >= 100)
