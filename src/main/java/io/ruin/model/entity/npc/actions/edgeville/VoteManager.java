        ...
        NPCAction.register(1815, "claim-votes", (player, npc) -> {
            Votes.claim(player, npc, (claimed, runelocus) -> {
                ...
                if(player.voteMysteryBoxReward >= 3) {
                    ...
                    bank = player.getInventory().isFull();
                    if(bank) {
                        player.getBank().add(995, 1000000);
                    } else {
                    ...
                    player.sendMessage(Color.COOL_BLUE.wrap("You receive a voting mystery box, double xp scroll, vote lottery ticket and 1m cash for voting on all 3 sites" + (bank ? " which has been deposited into your bank" : "") + "!"));
                }
            });
        });
        startEvent(e -> {
            while(true) {
                ...
                if(voteMysteryBoxesClaimed > 1) {
                    Broadcast.WORLD.sendNews(Icon.ANNOUNCEMENT, "Another " + voteMysteryBoxesClaimed + " players have claimed their FREE Voting Mystery Box and Double Experience Scroll! Type ::vote and claim yours now!");
