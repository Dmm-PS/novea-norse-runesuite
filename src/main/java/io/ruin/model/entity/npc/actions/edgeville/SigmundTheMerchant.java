...
public class SigmundTheMerchant {
    ...
    static {
        if(World.isPVP()) {
            ...
            ItemDef.get(7510).sigmundBuyPrice = 1000; //Dwarven rock cake
        ...
        if(World.isEco()) {
            NPCAction.register(SIGMUND, "buy-items", (player, npc) -> {
                if(player.getGameMode().isIronMan()) {
                    ...
                    stock.add(new ShopItem(3157, 1));        // karambwanji vessel
                    ...
                    stock.add(new ShopItem(10828, 75000));       // Helm of neitiznot
                    stock.add(new ShopItem(4081, 24000));       // Salve Amulet
                    stock.add(new ShopItem(3150, 375));       // Karambwan
