...
public class PlayerCombat extends Combat {
    ...
    public boolean canAttack(Entity target, boolean message) {
        ...
        if(target.player != null) {
            ...
            //Donator Boss
            if (target.npc.getId() == 7411 || target.npc.getId() == 7412 || target.npc.getId() == 7414 || target.npc.getId() == 7415) {
    ...
    private boolean salveAmulatBoost(Player player, Hit hit, Entity entity) {
        final int salveAmulet = 4081;
        final int salveAmuletImbued = 12017;

        if (hit.attackStyle != null && target.npc != null) {
            Item amulet = player.getEquipment().get(Equipment.SLOT_AMULET);
            if (amulet == null) {
                return false;
            }

            Optional<String> npc = UNDEAD_NPCS.stream().filter(Objects::isNull).filter(s -> s.equalsIgnoreCase(entity.npc.getDef().name) || Utils.containsIgnoreCase(entity.npc.getDef().name, s)).findAny();
    ...
    private void preTargetDefend(Hit hit, Entity target) {
        ...
        && !salveAmulatBoost(player, hit, target)) {
            // do nothing
        }
    ...
    public void startDeath(Hit killHit) {
        ...
        player.startEvent(event -> {
            ...
            if(player.deathEndListener != null) {
                ...
                player.getMovement().teleport(3101, 3494, 0);
