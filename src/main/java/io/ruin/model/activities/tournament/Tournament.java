...
public class Tournament {
    ...
    /**
     * choose between DMM tribrid or range
     */
    public static boolean TOGGLE_SPECIAL = false;

    /**
     * Tournament bounds
     */
    public static final Bounds JOIN_BOUNDS = World.isPVP() ? new Bounds(3077, 3488, 3081, 3497, 0) : new Bounds(3107, 3511, 3111, 3517, 0);
    public static final Bounds LEAVE_BOUNDS = World.isPVP() ? new Bounds(3077, 3488, 3081, 3497, 0) : new Bounds(3110, 3511, 3110, 3515, 0);
    ...
    public static final Bounds TOURNAMENT_BOUNDS = new Bounds(3264, 4928, 3327, 4996, 0);
     ...
     */
    public static long ticksUntilStart;
     ...
     */
    public static boolean tournamentInProgress = false;
    ...
    private static final int FIRST_PLACE_BM = World.isPVP() ? 10000 : 10_000_000;
    private static final int SECOND_PLACE_BM = World.isPVP() ? 7500 : 5_000_000;
    private static final int THIRD_PLACE_BM = World.isPVP() ? 5000 : 3_000_000;
    private static final int DEFAULT_BM = 1000;
        ...
        player.dialogue(new NPCDialogue(npc, "Would you like to join the tournament?"),
                new OptionsDialogue("Would you like to join the tournament?",
                        new Option("Yes, sign me up!", () -> player.getMovement().startTeleport(event -> {
                            if (Tournament.TOGGLE_SPECIAL) {
                                schedule = TournamentSchedule.DMM1;
                            }
                            player.lock();
                            ...
                            if (TOGGLE_SPECIAL && schedule.optionalPresets != null) {
                                player.startEvent(e -> player.dialogue(new OptionsDialogue("Pick your poison!",
                                        new Option("DMM Range/Melee", () -> {
                                            player.tournamentPreset = schedule.optionalPresets[1];
                                            TournamentPreset.use(player, TournamentPreset.DMM_RANGE_MELEE);
                                            player.unlock();
                                        }),
                                        new Option("DMM Tribrid", () -> {
                                            player.tournamentPreset = schedule.optionalPresets[0];
                                            TournamentPreset.use(player, TournamentPreset.DMM_TRIBRID);
                                            player.unlock();
                                        }))));
                            } else {
    ...
    public static long secondsUntilStart() {
        return secondsUntilStart(ticksUntilStart);
    }
    ...
    private static void rewardPlayer(Player player, String place, String status, int bloodMoneyAmount, int ticketAmount, boolean topThree) {
        ...
        player.sendMessage("<col=6f0000>You receive " + bloodMoneyAmount + " coins for your participation.");
    ...
    private static void resetPlayer(Player player) {
        World.startEvent(event -> {
            ...
            if (player.tournamentPouch) {
                player.getRunePouch().set(0, null);
                player.getRunePouch().set(1, null);
                player.getRunePouch().set(2, null);
            ...
            if (player.cachedRunePouchTypes != 0) {
                ...
                int firstRuneType = Config.RUNE_POUCH_LEFT_TYPE.get(player);
                if (firstRuneType != 0) {
                    Rune rune = Rune.VALUES[firstRuneType - 1];
                    player.getRunePouch().set(0, new Item(rune.getId(), Config.RUNE_POUCH_LEFT_AMOUNT.get(player)));
                }

                int secondRuneType = Config.RUNE_POUCH_MIDDLE_TYPE.get(player);
                if (secondRuneType != 0) {
                    Rune rune = Rune.VALUES[secondRuneType - 1];
                    player.getRunePouch().set(1, new Item(rune.getId(), Config.RUNE_POUCH_MIDDLE_AMOUNT.get(player)));
                }

                int thirdRuneType = Config.RUNE_POUCH_RIGHT_TYPE.get(player);
                if (thirdRuneType != 0) {
                    Rune rune = Rune.VALUES[thirdRuneType - 1];
                    player.getRunePouch().set(2, new Item(rune.getId(), Config.RUNE_POUCH_RIGHT_AMOUNT.get(player)));
                }
            ...
            player.tempUseRaidPrayers = false;
            player.getCombat().resetKillers();
                ...
                while (true) {
                    if ((ticksUntilStart - Server.currentTick()) * 0.6 == 1800) {
                        Broadcast.WORLD.sendNews(Icon.ANNOUNCEMENT, "Tournament", "In 30 minutes a <col=006600>"
                                + (Tournament.TOGGLE_SPECIAL ? "DMM Style" : schedule.presetName) + "</col> <col=1e44b3>tournament is starting! Type ::tournament to sign up now!");
                        ...
                        MANAGER.animate(862);
                        TournamentEmbedMessage.sendDiscordMessage((Tournament.TOGGLE_SPECIAL ? "DMM Style" : schedule.presetName), "15 minutes");
                        ...
                        Broadcast.WORLD.sendNews(Icon.ANNOUNCEMENT, "Tournament", "In 1 minute a <col=006600>"
                                + (Tournament.TOGGLE_SPECIAL ? "DMM Style" : schedule.presetName) + "</col> <col=1e44b3>tournament is starting! Type ::tournament sign up now!");
                        MANAGER.forceText("1 minutes until the tournament event starts!");
                        TournamentEmbedMessage.sendDiscordMessage((Tournament.TOGGLE_SPECIAL ? "DMM Style" : schedule.presetName), "1 minutes");
                        MANAGER.animate(862);
                        ...
                        Broadcast.WORLD.sendNews(Icon.ANNOUNCEMENT, "Tournament", "In 30 seconds a <col=006600>"
                                + (Tournament.TOGGLE_SPECIAL ? "DMM Style" : schedule.presetName) + "</col> <col=1e44b3>tournament is starting! Quickly type ::tournament to sign up now!");
...
/*        if (!(player.getPosition().inBounds(FIGHT_BOUNDS) && (player.tournamentTarget != null &&
                pTarget == player.tournamentTarget) && player.canAttackTournamentTarget)) {
            System.out.println("------");
            System.out.println("In fight bounds: " + player.getPosition().inBounds(FIGHT_BOUNDS));
            System.out.println("tournamentTarget: " + player.tournamentTarget != null);
            System.out.println("are you my target: " + (pTarget == player.tournamentTarget));
            System.out.println("Can i attack my tournament target: " + player.canAttackTournamentTarget);
        }*/

        return player.getPosition().inBounds(FIGHT_BOUNDS) && (player.tournamentTarget != null &&
                pTarget == player.tournamentTarget) && player.canAttackTournamentTarget;
    }
