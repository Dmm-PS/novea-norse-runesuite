...
public class BossEvent extends JournalEntry {
    ...
    private static NPC spawnNext() {
        ...
        boss.deathEndListener = (DeathListener.SimpleKiller) killer -> {
            ...
            if (firstPlaceKiller != null) {
                firstPlaceKiller.player.getInventory().addOrDrop(new Item(995, 10000000));
            }

            if (secondPlaceKiller != null) {
                secondPlaceKiller.player.getInventory().addOrDrop(new Item(995, 5000000));
            }

            if (thirdPlaceKiller != null) {
                thirdPlaceKiller.player.getInventory().addOrDrop(new Item(995, 3000000));
            }
    ...
    private static double bloodMoneyPerDamage(Player player) {
        if (player.isGroup(PlayerGroup.GODLIKE_DONATOR)) {
            return 9.0;
        } else if (player.isGroup(PlayerGroup.MASTER_DONATOR)) {
            return 8.0;
    ...
    static {
        World.startEvent(e -> {
            ...
            if (now.getMinute() > 30)
                firstSpawn = now.plus(Duration.ofHours(2)).withMinute(0);
            else
            ...
            while (true) {
                NPC npc = spawnNext();
                nextSpawn = Server.getEnd(120 * 100);
