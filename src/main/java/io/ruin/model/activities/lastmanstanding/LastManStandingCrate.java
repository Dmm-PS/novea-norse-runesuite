...
public enum LastManStandingCrate { //spawns 45 seconds after game start and again after 60 seconds
    ...
    static {
        ObjectAction.register(29081, "search", LastManStandingCrate::openChest);
    }
    ...
    public static void spawnChest(DynamicMap LMSMap, List<Player> playersFighting) {
        LastManStandingCrate lmsCrate = randomCrate();
        GameObject.spawn(29081, LMSMap.convertPosition(lmsCrate.spawnPosition), 10, 0);
        playersFighting.forEach(player -> player.getPacketSender().sendMessage("A magical loot crate has appeared " + lmsCrate.spawnHint, "", 14));
    }
