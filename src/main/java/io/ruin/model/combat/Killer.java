...
//    private static final Bounds EDGEVILLE_FARM_SKIP_BOUNDS = new Bounds(2996, 3430, 3187, 3599, 0);
    ...
    public void reward(Player pKilled, boolean bhTarget) {
        ...
        wildernessPoints *= WildernessPoints.wildernessLevelModifier(player);
        if (World.doublePkp)
            wildernessPoints *= 2;
         ...
         */
        if(World.isEco()) {
            int goldAmount = Random.get(100_000, 200_000);
        ...
        if (World.isEco() && World.wildernessDeadmanKeyEvent) {
