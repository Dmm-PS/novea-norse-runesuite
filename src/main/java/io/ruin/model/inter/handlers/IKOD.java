    ...
    public static void forLostItem(Player player, Killer killer, Consumer<Item> dropConsumer) {
        ...
        for(Item item : items) {
            ...
            if(upgrade != null) {
                ...
                //If the item is an upgradeable drop coins of 5% upgrade cost no matter what level wildy
                currency.incrementAmount(World.isPVP() ? upgrade.bmUpgradeCost : (long) (upgrade.coinUpgradeCost * .05));
                if(!regularDef.tradeable) {
                    //if below lvl 20 wild and its untradeable keep unimbued item + 5% cost is dropped
                    if(World.isPVP() || player.wildernessLevel < 20) {
                        keepItems.add(new Item(upgrade.regularId));
                    }
                    continue;
                }
                //if tradeable unupgraded drop item + 5% gold
