...
public class Store {
    ...
    private static void claimCredits0(Player player, Item item) {
        ...
        Server.forumDb.execute(new DatabaseStatement() {
            ...
            public void execute(Connection connection) throws SQLException {
                ...
                try {
                    statement = connection.prepareStatement("SELECT * FROM core_members WHERE member_id = ?", ResultSet.TYPE_SCROLL_SENSITIVE, ResultSet.CONCUR_UPDATABLE);
    ...
    private static void updateCredits(Player player, int credits, ResultSet resultSet) throws SQLException {
        resultSet.updateInt("member_id", player.getUserId());
        resultSet.updateString("name", player.getName()); //eh idk why this is stored lol
            ...
            public void execute(Connection connection) throws SQLException {
                ...
                try {
                    ...
                    while(resultSet.next()) {
                        ...
                        resultSet.updateRow();
                        if (itemId == 1233) { // Norse Starter package
